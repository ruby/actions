...
# Windows (mswin).
#
# Vendored copy of tool/binary-package.rb proposed for ruby/ruby, where
# it backs the `binary-package` target in win32/Makefile.sub.  Here it
# is applied from the outside to a stage made by `nmake install-nodoc
# DESTDIR=<stage>`, so that every target branch can be packaged the
# same way without patching its build system.  Keep this copy in sync
