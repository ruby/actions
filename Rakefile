...
task "snapshot:stable" do
  FileUtils.mkdir "pkg" unless File.directory?("pkg")
  `cd ruby && git checkout ruby_2_6 && git pull origin ruby_2_6`
  `ruby ruby/tool/make-snapshot -archname=stable-snapshot -srcdir=ruby pkg stable`
